...
func Migrate(db *sql.DB) error {
	...
	// Add deleted_at to accounts (soft delete)
	if !columnExists(db, "accounts", "deleted_at") {
		if _, err := db.Exec(`ALTER TABLE accounts ADD COLUMN deleted_at DATETIME`); err != nil {
			return err
		}
	}
